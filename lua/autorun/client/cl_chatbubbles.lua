...
hook.Add('PostDrawTranslucentRenderables', 'chatbubbles_PostDrawTranslucentRenderables',
	function(_, isDrawSkybox, isDraw3DSkybox)
	...
	for id, b in pairs(bubbles) do
		if IsValid(b.ent) then
			...
			if eyePos:DistToSqr(pos) < 4000000 then
...
net.Receive('chatbubbles.show', function(_)
	local ent = net.ReadEntity()
	local str = net.ReadString()

	if not IsValid(ent) then return end

	local fg = Color(net.ReadUInt(8), net.ReadUInt(8), net.ReadUInt(8), net.ReadUInt(8))
	local bg = Color(net.ReadUInt(8), net.ReadUInt(8), net.ReadUInt(8), net.ReadUInt(8))
